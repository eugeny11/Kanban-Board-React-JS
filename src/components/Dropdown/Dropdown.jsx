...
const Dropdown = (props) => {
  ...
  const handleClick = (e) => {
    ...
    const chosenFoundGoal = editedGoals.find(
      (goal) => goal.id === chosenGoalId
    );
  ...
  if (isFormVisible) {
    return (
      <form onSubmit={onSubmit}>
        <select className="select" ref={chosenGoal}>
  ...
  return (
    <button
      disabled={!listOfDropdownGoals.length ? true : false}
      onClick={handleAddNewClick}
      className="button add-goal-button"
    >
      Add goal
    </button>
  );
